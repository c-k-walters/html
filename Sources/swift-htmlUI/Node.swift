...
protocol Element: Node {
	...
	var content: [any Node] { get }
}

extension Element {
	var body: some Node {
		ElementNode(element: self) {
			content
		}
	}
}
...
struct ElementNode<E: Element>: LeafNode, Node {
	var element: E
	var content: [any Node]

	init(element: E, @NodeBuilder content: () -> [any Node]) {
		self.element = element
		self.content = content()
	}

	func render() -> Html {
		let inner = content.reduce(into: "") { partialResult, next in
			// TODO: indent here
			partialResult.append(next._render())
		}

		// TODO: newlines and formatting
...
struct NodeBuilder {
	...
	public static func buildIf<N: Node>(_ component: N?) -> [any Node] {
	...
	public static func buildEither<N: Node>(first: N) -> [any Node] {
		[
			first
		]
	}

	public static func buildEither<N: Node>(second: N) -> [any Node] {
		[
			second
		]
	}
...
// some struct that is ultimately passes in a response?
